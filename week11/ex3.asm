.eqv HEADING 0xffff8010 	# Integer: An angle between 0 and 359
 				# 0 : North (up)
				# 90: East (right)
 				# 180: South (down)
 				# 270: West (left)
.eqv MOVING 0xffff8050 		# Boolean: whether or not to move
.eqv LEAVETRACK 0xffff8020 	# Boolean (0 or non-0):
 				# whether or not to leave a track
.eqv WHEREX 0xffff8030 		# Integer: Current x-location of MarsBot
.eqv WHEREY 0xffff8040 		# Integer: Current y-location of MarsBot
.text 
main: 	jal goDOWN
 	nop
 	jal ROTATE
 	nop
 	jal GO
 	nop
 	jal sleep8000
 	nop
 	jal goRIGHT
 	nop
 	jal sleep2000
 	nop
 	jal TRACK
 	nop
 	jal sleep2000
 	nop
 	jal goUP
 	nop
 	jal sleep1000
 	nop
 	jal goLEFT
 	nop
 	jal sleep1000
 	nop
 	jal goDOWN
 	nop
 	jal sleep8000
 	nop
 	jal goLEFT
 	nop
 	jal sleep1000
 	nop
 	jal goUP
 	nop
 	jal sleep4000
 	nop
 	jal goRIGHT
 	nop
 	jal sleep8000
 	nop
 	jal goUP
 	nop
 	jal sleep4000
 	nop
 	jal goLEFT
 	nop
 	jal sleep1000
 	nop
 	jal goDOWN
 	nop
 	jal sleep8000
 	nop
 	jal goRIGHT
 	nop
 	jal sleep1000
 	nop
 	jal STOP
 	nop
end_main:	
	addi $v0, $0, 10
	syscall

goASKEW:addi $a0, $zero, 45 	# Marsbot rotates 45* 
	add $s0, $0, $ra
 	jal ROTATE 
 	nop
 	add $ra, $s0, $0
 	jr $ra
 	nop
goUP: 	addi $a0, $zero, 0 	# Marsbot rotates 0* 
	add $s0, $0, $ra
 	jal ROTATE
 	nop
 	add $ra, $s0, $0
 	jr $ra
 	nop
goDOWN:	addi $a0, $zero, 180 	# Marsbot rotates 180* 
	add $s0, $0, $ra
 	jal ROTATE 
 	nop
 	add $ra, $s0, $0
 	jr $ra
 	nop
goRIGHT: addi $a0, $zero, 90 	# Marsbot rotates 90* 
	add $s0, $0, $ra
 	jal ROTATE
 	nop
 	add $ra, $s0, $0
 	jr $ra
 	nop
goLEFT: addi $a0, $zero, 270 	# Marsbot rotates 270* 
	add $s0, $0, $ra
 	jal ROTATE
 	nop
 	add $ra, $s0, $0
 	jr $ra
 	nop

sleep1000: addi $v0,$zero,32 	# Keep running by sleeping in 1000 ms 
 	li $a0,1000 
 	syscall
 	add $s0, $0, $ra
 	jal UNTRACK 		# keep old track
 	nop
 	jal TRACK 		# and draw new track line
 	nop
 	add $ra, $s0, $0
 	jr $ra
 	nop
sleep4000:	
	addi $v0,$zero,32 	# Keep running by sleeping in 1000 ms
 	li $a0,4000 
 	syscall
 	add $s0, $0, $ra
 	jal UNTRACK 		# keep old track
 	nop
 	jal TRACK 		# and draw new track line
 	nop
 	add $ra, $s0, $0
 	jr $ra
 	nop
 	sleep2000:	
	addi $v0,$zero,32 	# Keep running by sleeping in 1000 ms
 	li $a0,2000 
 	syscall
 	add $s0, $0, $ra
 	jal UNTRACK 		# keep old track
 	nop
 	jal TRACK 		# and draw new track line
 	nop
 	add $ra, $s0, $0
 	jr $ra
 	nop
sleep8000:	addi $v0,$zero,32 	# Keep running by sleeping in 2000 ms 
 	li $a0,8000 
	syscall 
 	add $s0, $0, $ra
 	jal UNTRACK 		# keep old track
 	nop
 	jal TRACK 		# and draw new track line
 	nop 
 	add $ra, $s0, $0
 	jr $ra
 	nop
#-----------------------------------------------------------
# GO procedure, to start running
# param[in] none
#-----------------------------------------------------------
GO: 	li $at, MOVING 		# change MOVING port
 	addi $k0, $zero,1 	# to logic 1,
 	sb $k0, 0($at) 		# to start running
 	nop 
 	jr $ra
 	nop
#-----------------------------------------------------------
# STOP procedure, to stop running
# param[in] none
#-----------------------------------------------------------
STOP: 	li $at, MOVING 		# change MOVING port to 0
 	sb $zero, 0($at) 	# to stop
 	nop
 	jr $ra
 	nop
#-----------------------------------------------------------
# TRACK procedure, to start drawing line 
# param[in] none
#----------------------------------------------------------- 
TRACK: 	li $at, LEAVETRACK 	# change LEAVETRACK port
 	addi $k0, $zero,1 	# to logic 1,
 	sb $k0, 0($at) 		# to start tracking
 	nop
 	jr $ra
 	nop 
#-----------------------------------------------------------
# UNTRACK procedure, to stop drawing line
# param[in] none
#----------------------------------------------------------- 
UNTRACK:li $at, LEAVETRACK 	# change LEAVETRACK port to 0
 	sb $zero, 0($at) 	# to stop drawing tail
 	nop
 	jr $ra
	nop
#-----------------------------------------------------------
# ROTATE procedure, to rotate the robot
# param[in] $a0, An angle between 0 and 359
# 0 : North (up)
# 90: East (right)
# 180: South (down)
# 270: West (left)
#-----------------------------------------------------------
ROTATE:	li $at, HEADING 	# change HEADING port
 	sw $a0, 0($at) 		# to rotate robot
 	nop
 	jr $ra
 	nop
